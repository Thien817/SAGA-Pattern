...
public sealed class OrderService(IOrderRepository orderRepository) : IOrderService
{
    ...
    private static OrderDto MapToDto(OrderWithItemsRecord orderWithItems)
    {
        return new OrderDto(
            ...
            orderWithItems.Items.Select(x => new OrderItemDto(x.OrderItemId, x.ProductId, x.Quantity, x.UnitPrice, x.LineTotal)).ToList());
    }
}
